...
public class Problem2Solver implements ALPSolver {
    ...
    public ALPSolution solve(ALPInstance instance) {
        try {
            ...
            IloNumVar makespan = cplex.numVar(0, Integer.MAX_VALUE, "makespan");
    ...
    private ALPSolution solveWithHeuristic(ALPInstance instance) {
        ...
        // Liste des avions triée par plus tôt temps d'atterrissage
        List<AircraftData> sortedAircraft = new ArrayList<>(instance.getAircraft());
        sortedAircraft.sort((a1, a2) -> Integer.compare(a1.getEarliestLandingTime(), a2.getEarliestLandingTime()));
        ...
        // Calculer le makespan (objectif)
        int makespan = 0;
        for (int i = 0; i < n; i++) {
            makespan = Math.max(makespan, landingTimes[i]);
        }
        ...
        return new ALPSolution(instance, landingTimes, runwayAssignments, 
                             makespan, solveTime, "Problem 2: Minimizing Makespan (Heuristic)");
    }
